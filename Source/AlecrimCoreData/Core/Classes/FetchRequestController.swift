...
public final class FetchRequestController<T: NSManagedObject> {
    ...
    //
    internal fileprivate(set) lazy var delegate = FetchRequestControllerDelegate<T>()
    ...
    public fileprivate(set) lazy var underlyingFetchedResultsController: NSFetchedResultsController<T> = {
